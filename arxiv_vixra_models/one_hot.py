...
def one_hot_encoding(s: str,
                     char_to_idx: Union[str, DF, Dict[str, int]],
...
def one_hot_decoding(tensor: Tensor,
                     idx_to_char: Union[str, DF, Dict[int, str]]) -> str:
    ...
    stripped_text = ''.join(idx_to_char[c.argmax().item()] for c in tensor).strip()
