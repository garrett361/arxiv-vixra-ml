...
def str_to_one_hot(
    ...
    Description
    ----------
    Given the char_to_idx_dict dictionary mapping chars to ints, return the one-hot
    ...
    first.  Resulting shape is (seq_len, len(char_to_idx_dict)).

    Parameters
    ----------
    s : str
        Text to to be embedded
    char_to_idx_dict : dict
    ...
    input_size = len(char_to_idx_dict)
    s_len = len(s)
    if seq_len is None:
        seq_len = s_len
    else:
        if s_len > seq_len:
            ...
            s = (seq_len - s_len) * " " + s
    s_int = [char_to_idx_dict.get(c, BLANK_IDX) for c in s]
    s_tensor = torch.zeros((seq_len, input_size))
    s_tensor[torch.arange(seq_len), s_int] = 1.0
    return s_tensor


def one_hot_to_str(tensor: Tensor, idx_to_char_dict: Dict[int, str]) -> str:
    """Decode a one-hot-encoded tensor.

    Description
    ----------
    Given the idx_to_char_dict dictionary mapping ints to chars, return the string
    ...
    text : str
        Decoded string.
    """
    text = "".join(idx_to_char_dict.get(c.argmax().item(), "<UNK>") for c in tensor)
    return text


def char_to_idx_dict_from_df(
    df: DataFrame,
    char_col: str = "char",
    idx_col: str = "idx",
    check_normalization=True,
) -> dict:
    """Generates char_to_idx_dict dictionary from DataFrame.

    Parameters
    ----------
    df : DataFrame
        Dataframe holding char-idx mapping
    char_col : str, default 'char'
    ...
    for _, row in df.iterrows():
        char_to_idx_dict[row[char_col]] = row[idx_col]
    if check_normalization:
        blank_check = char_to_idx_dict[" "]
        assert (
            blank_check == BLANK_IDX
        ), f"Blank space expected to map to {BLANK_IDX}, not {blank_check}."
    return char_to_idx_dict


def idx_to_char_dict_from_df(
    df: DataFrame,
    char_col: str = "char",
    idx_col: str = "idx",
    check_normalization=True,
) -> dict:
    """Generates idx_to_char_dict dictionary from DataFrame.
    ...
    idx_to_char_dict = {}
    for _, row in df.iterrows():
        idx_to_char_dict[row[idx_col]] = row[char_col]
    if check_normalization:
