...
def string_to_ints(s: str,
                 word_to_idx: Union[str, DF, Dict[str, int]],
                 seq_len: Optional[Union[None, int]] = None,
                 check_normalization: Optional[bool] = True) -> Tensor:
    ...
    if check_normalization:
        assert s == text_normalizer(s), 'String not normalized as expected, apply text_normalizer first.'
    ...
    if not isinstance(word_to_idx, dict):
        if isinstance(word_to_idx, str):
            ...
            word_df = word_to_idx
        word_to_idx = dict(zip(word_df['word'], np.arange(2, len(word_df) + 2)))
    ...
    if seq_len > s_len:
        s_int_list = (seq_len - s_len) * [0] + [word_to_idx.get(w, 1) for w in s_list]
    else:
        s_int_list = [word_to_idx.get(w, 1) for w in s_list[:seq_len]]
    s_int_tensor = torch.tensor(s_int_list)
    return s_int_tensor

def ints_to_string(tensor: Tensor,
                   idx_to_word: Union[str, DF, Dict[int, str]]) -> str:
    ...
    if not isinstance(idx_to_word, dict):
        if isinstance(idx_to_word, str):
            ...
            word_df = idx_to_word
        idx_to_word = dict(zip(np.arange(2, len(word_df) + 2), word_df['word']))
